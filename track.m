...
function info = track(xyzs,maxdisp)
%xyzs = read_gdf('/Users/blair/data/test_data.gdf');
%maxdisp = 4.5
dd = length(xyzs(1,:));
dim = dd - 1;
    ...
    if m > 0
        ...
        if notnsqrd
          ...
          %disp(size(resx))
            ...
            %%%%damon changes here 10/4/04 -- to end of next comment
            xmat=ones(ntrack,1)*[1:m];
            ymat=[1:ntrack]'*ones(1,m);
            %xmat = zeros(ntrack,m);
            %count = 0;
            %for k=1:ntrack
            %    for j=1:m
            ...
            rowtot = zeros(n,1);
            %keyboard;
            rowtot(wh) = sum(ltmax,2); % damon changed '1' to '2' in this line, 10/4/04
            
            
            if ntrack > 1 
                coltot = sum(ltmax,1);  % damon changed '2' to '1' in this line, 10/4/04
            else
                coltot = ltmax;
            end
            which1 = zeros(n,1);
            ...
            ntrk = fix( n - sum(rowtot(find(rowtot == 0))));
            w= find( rowtot == 1) ;
            ngood = length(w);
        ...
        if nontrivial
            ...
            for j=0:ydim-1
                ...
                for d=1:dim
                    %distq
                    distq = distq + (xyi(labelx,d) - pos(labely(j+1),d)).^2;
                    %distq    
                end
...
%                end
                ...
                resx(ispan,labely(bonds(minbonds,2)+1)) = eyes(labelx(bonds(minbonds,1)+1));
...
%            disp(size(resx));
  ...
  %  nok
    ...
    %r_eq_no = (resx(ispan,:) == -1);
    nadd = length(mem)-length(resx(ispan,:));
    if nadd ~= 0
        mem = [mem,zeros(nnew,1)];
    end    

    num = find(resx(ispan,:) == -1);
    if ~isempty(num)
        mem(num(:)) = 1;   
    end
